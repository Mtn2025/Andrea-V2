...
# New Routes
from app.api import routes_v2, routes_simulator, routes_simulator_v2, routes_telephony, routes_admin
...
app.include_router(routes_simulator_v2.router, prefix="/ws/simulator/v2", tags=["Simulator V2"])
...
app.include_router(routes_v2.router, prefix=settings.API_V1_STR) # Deprecated redirects
    ...
    # v1 Routers
app.include_router(dashboard.router, tags=["dashboard"]) # Dashboard router seems to have prefix /dashboard or /api/dashboard? Need to check but safer to trust router internal prefix if standard.
...
async def liveness():
    """
    Liveness probe: responde 200 si la aplicación está viva (sin comprobar BD/Redis).
    Para readiness (BD + Redis) usar GET /api/system/health.
    """
    return {"status": "ok"}
